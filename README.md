...
项目集成了 JWT 鉴权、语义缓存（Semantic Cache）、全局异常处理、Loguru 规范化日志、APScheduler 数据库定时备份以及跨环境的
Docker 自动化部署方案。
...
并在启动 Docker 时使用 `--env-file .env.siliconflow` 参数指定环境变量文件。
